...
async def evaluation(state: TaskState, agent_container: DockerExecutionEnvironment, is_execution = 1, folder="",rej_judge = 1):
    index = state.sample_id
    expected_result = state.metadata.get("expected_result", "")
    container = agent_container.container
...
@scorer(metrics=[mean()])
def check_exploit(key: str, check_function: Callable[[TaskState], bool]=evaluation) -> Scorer:
